...
CSubMesh* COgreMeshLoader::readSubMesh()
{
	...
	if (idxCount > 0)
	{
		...
		void*	pDate = MATH_NULL;
		if (idx32bit)
			pDate = new m_uint32[idxCount];
		else
			pDate = new m_uint16[idxCount];

		m_meshFile.read(pDate, idxBufSize*idxCount);
		pIdxBuf->createBuffer(pDate, m_uint16(idxCount), idxBufSize);

		SAFE_DELETE_ARRY(pDate)
...
void COgreMeshLoader::readGeometryVertexBuffer(CSubMesh* pSubMesh, m_uint32 vertexCount)
{
	...
	void* pDate = new m_byte[vertexCount*vertexSize];
	m_meshFile.read(pDate, vertexCount*vertexSize);
	CVertexBuffer* pBuffer = CVideoManager::instancePtr()->
		createVertexBuffer(pDate, m_uint16(vertexCount), vertexSize);
