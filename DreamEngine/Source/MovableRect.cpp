...
void CTestMovable::onCreate()
{
	...
	CVertexBuffer* pVertexBuffer = CVideoManager::instancePtr()
		->createVertexBuffer(data, 4, sizeof(strData), TRIANGLE_STRIP);
	pRenderData->addVertexBuffer(0, pVertexBuffer);

	m_byte idx[4] = {0,1,2,3};
	CIndexBuffer* pIndxBuffer = CVideoManager::instancePtr()->createIndexBuffer(TRIANGLE_STRIP);
	...
	//CTexture* pTexture = CVideoManager::instancePtr()->createTextureFromFile("xx","monguger.tga");
	//pTexture->setTexCoordLayer(0);
	///CShaderParam* pParam = pProgram->getShaderParamTable()->getParam("s_texture");
	//pParam->setAddressVaule(pTexture);
