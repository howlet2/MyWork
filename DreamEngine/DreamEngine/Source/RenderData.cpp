...
{
	for (VertexStream::iterator iter=m_vertexStream.begin(); iter!=m_vertexStream.end(); ++iter)
	{
		if (!CVideoManager::instancePtr()->destoryVertexBuffer(iter->second->getName()))
		{
			iter->second->destroy();
	...
	for (IndexStream::iterator iter=m_indexStream.begin(); iter!=m_indexStream.end(); ++iter)
	{
		if (!CVideoManager::instancePtr()->destoryIndexBuffer((*iter)->getName()))
		{
			(*iter)->destroy();
			SAFE_DELETE((*iter));
		}
